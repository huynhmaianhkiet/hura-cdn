/* Beer: Before & After Slider */ ! function (t, e) {
...
function iniHuraAppsBlocks($) {
  //Tabs Section
  $('.hura-tab-title').click(function () {
    var blockID = $(this).closest('.hura-tabs-container').data('block');
    var tabID = $(this).data('tab');
    var $block = $('.hura-tabs-container-' + blockID);
    $block.find('.hura-tab-title').removeClass('active');
    $block.find('.hura-tab-content').removeClass('active');
    $(this).addClass('active');
    $block.find('.hura-tab-content-' + tabID).addClass('active');
  });

  //FAQs Section
  $('.hura-faq-container .hura-question').click(function () {
    var blockID = $(this).closest('.hura-faq-container').data('block');
    var $faq = $('.hura-faq-container-' + blockID);
    if ($(this).closest('.hura-faq').hasClass('active')) {
      $faq.find('.hura-faq').removeClass('active');
      $(this).closest('.hura-faq').find('.hura-answer').slideUp('500');
    } else {
      $faq.find('.hura-faq.active').find('.hura-answer').slideUp('500');
      $faq.find('.hura-faq').removeClass('active');
      $(this).closest('.hura-faq').addClass('active');
      $(this).closest('.hura-faq').find('.hura-answer').slideDown('500');
    }
  });
  ...
  //Mailchimp Ajax
  $('.hura-mailchimp-signup-form').submit(function (e) {
    e.preventDefault();
    var $section = $(this).closest('.hura-mailchimp-container')
    var $form = $(this);
    $.ajax({
      type: 'POST',
      ...
      success: function (data) {
        ...
        $section.find('.hura-mailchimp-response').html(data.msg).show();
      }
    });
  });

  //Swiper Slider
  if ($('.has-hura-swiper').length > 0) {
    ...
    //Logo list
    if ($('.hura-slide-logo-list-wrapper').length > 0) {
      $('.hura-slide-logo-list-wrapper').each(function (index, value) {
        var logoPerView = $(this).data('pagination');
        var delayTime = parseInt($(this).data('delay')) * 1000;
        var swiperClass = '.swiper-container-' + $(this).data('block-id');
    ...
    //Youtube list
    if ($('.hura-slide-youtube-wrapper').length > 0) {
      $('.hura-slide-youtube-wrapper').each(function (index, value) {
        var logoPerView = $(this).data('pagination');
        var delayTime = parseInt($(this).data('delay')) * 1000;
        var swiperClass = '.swiper-container-' + $(this).data('block-id');
        var swiper = new Swiper(swiperClass, {
  ...
  //Lightbox
  if ($('[data-fslightbox=gallery]').length > 0) {
    var lightbox_script = '<script src="//cdnjs.cloudflare.com/ajax/libs/fslightbox/3.3.1/index.min.js" crossorigin="anonymous" referrerpolicy="no-referrer"></script>';
    $('body').append(lightbox_script);
  }

}

var jHuraBlocks = document.createElement('script');
jHuraBlocks.setAttribute("src", "https://ajax.googleapis.com/ajax/libs/jquery/3.1.0/jquery.min.js");
jHuraBlocks.setAttribute("type", "text/javascript");
if (jHuraBlocks.readyState) {
  jHuraBlocks.onreadystatechange = function () {
    if (this.readyState == 'loaded' || this.readyState == 'complete') {
      iniHuraAppsBlocks(jQuery.noConflict(true));
    }
  };
} else {
  jHuraBlocks.onload = function () {
    iniHuraAppsBlocks(jQuery.noConflict(true));
  };
}
var d = document.getElementsByTagName("head")[0] || document.documentElement;
d.appendChild(jHuraBlocks);